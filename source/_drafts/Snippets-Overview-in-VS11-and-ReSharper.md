...
Let&rsquo;s start off with some facts:
...
Snippet functionality in VS is robust for certain. If it lacks anything, it&rsquo;s ease of use and even that&rsquo;s subjective. The process is to create a code snippet file &ndash; that is an XML file with a .snippet extension, drop it in the My Code Snippets folder, and then go to town with it. I obviously skipped over some details though, so just look at the _Walkthro__ugh &ndash; Creating a Code Snippet _link below for more.

## Snippet Designer

I would say that if you want to or have to (because maybe your employer is cheap and doesn&rsquo;t want to spring for R# licenses) stick with native snippets, you should at least go get the Snippet Designer. It&rsquo;s free, so if your employer doesn&rsquo;t have a group policy against installing free tools, then go get it from the link below, and if they do then consider a new employer or send them my way and I&rsquo;ll give them some financial justification with my 100 level economics that they won&rsquo;t be able to argue against.

[Snippet Designer](http://snippetdesigner.codeplex.com/) is a free plug-in for Visual Studio that makes it easier to create and modify your code snippets. One of the best features is the ability to just highlight a block of code you&rsquo;ve already written by conventional means, right click it, and choose to turn it into a snippet.
...
The bummer is that Snippet Designer doesn&rsquo;t work in VS11 yet. Sheesh, you&rsquo;d think we&rsquo;re still in beta or something!
...
*   the &ldquo;macros&rdquo; you can attach to place holders in templates and
